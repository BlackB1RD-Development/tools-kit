...
// Requires - Files
const { checkAvailability, get, backgrounds, colors, styles, consoles, reset } = require('../../src/Util.js');
...
class Logger {
  ...
  static _log(settings, ...args) {
    ...
    if (typeof settings.options.background === 'string') {
      if (!checkAvailability('backgrounds', settings.options.background)) return new Error(`The background name that you supplied isn't exist.\nList of all the available backgrounds: ${Object.keys(backgrounds).join(', ')}`);

      background = get('backgrounds', settings.options.background);
    }

    if (typeof settings.options.color === 'string') {
      if (!checkAvailability('colors', settings.options.color)) return new Error(`The color name that you supplied isn't exist.\nList of all the available colors: ${Object.keys(colors).join(', ')}`);

      color = get('colors', settings.options.color);
    }

    if (typeof settings.options.style === 'string') {
      if (!checkAvailability('styles', settings.options.style)) return new Error(`The style name that you supplied isn't exist.\nList of all the available styles: ${Object.keys(styles).join(', ')}`);

      style = get('styles', settings.options.style);
    }

    if (typeof settings.options.type === 'string') {
      if (!checkAvailability('consoles', settings.options.type)) return new Error(`The console name that you supplied isn't exist.\nList of all the available consoles types: ${consoles.join(', ')}`);
