...
const events = ['important', 'success', 'fatal', 'trace', 'err', 'debug', 'warn', 'info', 'figlet', 'log'];
...
class Logger {
  ...
  static _log(settings = {}, ...args) {
    if (!util.isObject(settings)) return this.error('The log settings must be an Object.');
    else if (!settings.options) return this.error('The log settings must have an options Object.');
    else if (settings.options && !util.isObject(settings.options)) return this.error('The log settings options must be an Object.');
    else {
      var background = '';
      var color = '';
      var style = '';
      ...
      if (typeof settings.options.background === 'string') {
        if (!checkAvailable('backgrounds', settings.options.background)) return this.error(`The background name that you supplied isn't exist.\nList of all the available backgrounds: ${Object.keys(backgrounds).join(', ')}`);
        else background = get('backgrounds', settings.options.background);
      }

      if (typeof settings.options.color === 'string') {
        if (!checkAvailable('colors', settings.options.color)) return this.error(`The color name that you supplied isn't exist.\nList of all the available colors: ${Object.keys(colors).join(', ')}`);
        else color = get('colors', settings.options.color);
      }

      if (typeof settings.options.style === 'string') {
        if (!checkAvailable('styles', settings.options.style)) return this.error(`The style name that you supplied isn't exist.\nList of all the available styles: ${Object.keys(styles).join(', ')}`);
        else style = get('styles', settings.options.style);
      }

      if (typeof settings.options.type === 'string') {
        if (!checkAvailable('consoles', settings.options.type)) return this.error(`The console name that you supplied isn't exist.\nList of all the available consoles types: ${Object.keys(consoles).join(', ')}`);
        else type = settings.options.type;
      }
      ...
      if (typeof settings.options.tag === 'string') tag = settings.options.tag;

      var prefix = time && tag ? `[${time} | ${tag}]: ` : time && !tag ? `[${time}]: ` : tag && !time ? `[${tag}]: ` : '';
      var logSettings = {
        name: settings.name,
        options: {
      ...
      loggersLogs.push(logSettings);
      emitter.emit(logSettings.name === 'error' ? 'err' : logSettings.name, logSettings);

      if (type === 'log') return console.log(`${color}${background}${style}${prefix}`, ...args, reset);
      else if (type === 'info') return console.info(`${color}${background}${style}${prefix}`, ...args, reset);
      else if (type === 'error') return console.error(`${color}${background}${style}${prefix}`, ...args, reset);
      else if (type === 'trace') return console.trace(`${color}${background}${style}${prefix}`, ...args, reset);
      else if (type === 'warn') return console.warn(`${color}${background}${style}${prefix}`, ...args, reset);
      else if (type === 'debug') return console.debug(`${color}${background}${style}${prefix}`, ...args, reset);
      else return console.log(`${color}${background}${style}${prefix}`, ...args, reset);
    }
  }
    ...
    try {
      ...
      const settings = {
        ...
        log: {
          name: 'figlet',
          options: {
            background: options.log && options.log.background ? options.log.background : options && options.background ? options.background : null,
            color: options.log && options.log.color ? options.log.color : options && options.color ? options.color : null,
            style: options.log && options.log.style ? options.log.style : options && options.style ? options.style : null,
      ...
      content.forEach(text => {
        figlet.text(text, settings.figlet, (err, data) => {
          if (err) return this.log({ background: 'black', color: 'red', type: 'log', tag: 'FIGLET ERROR' }, err);
      ...
      this.log({ background: 'black', color: 'red', type: 'log', tag: 'FIGLET ERROR' }, error);
    }
  }
  ...
  static log(...args) {
    ...
    const logSettings = {
      ...
      options: {
        ...
        tag: settings.options && settings.options.tag ? settings.options.tag : settings && settings.tag ? settings.tag : 'LOG'
      },
      args: args
    };
  ...
  static on(event, ...args) {
    if (!events.includes(event)) return this.error(event, ' is an invalid event name.\nAvailable events: ', events.join(', '));
