...
const init = () => {
  Object.keys(stylesObj).forEach(name => {
    return obj[name] = {
      get: function() {
        const styleObj = Object.defineProperties(function style(...args) {
          return style.styles.join('') + Util.format(...args) + reset;
        }, stylesObj);
...
class Styles {
  ...
  static background(name, ...args) {
    ...
    return background + Util.format(...args) + reset;
  }
  ...
  static style(name, ...args) {
    ...
    return style + Util.format(...args) + reset;
  }
  ...
  static stylify(options, ...args) {
    ...
    return background + color + style + Util.format(...args) + reset;
  }
  ...
  static map(text, colors, after) {
    ...
    text.split('').forEach(l => {
      if (l === ' ') transformed += ' ';
      else transformed += colors[i++ % after] + l + reset;
    });
  ...
  static rainbow(...args) {
    ...
    return this.map(Util.format(...args), mapColors, mapColors.length);
  }
   ...
   */
  static random(...args) {
    const mapColors = Object.values(backgrounds).concat(Object.values(colors), Object.values(styles));
