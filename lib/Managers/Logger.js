...
class Logger {
  /**
   * This method is not intended for use!
   * @access private
   * @private
   * @returns {Logger} The Logger class
   */
  static _log(settings, ...args) {
    if (!util.isObject(settings)) throw new Error('The log settings must be an Object.');
    else if (!settings.options) throw new Error('The log settings must have an options Object.');
    else if (settings.options && !util.isObject(settings.options)) throw new Error('The log settings options must be an Object.');
   ...
   * @param {Object} [options] The Logger.figlet() method options
   ...
   * @param {Object} [options.log] The Logger.log() method options
   * @param {String} [options.log.background] A string value that indicates the background color name to use when logging.
   * @param {String} [options.log.color] A string value that indicates the color name to use when logging.
   * @param {String} [options.log.style] A string value that indicates the style to use when logging.
   * @param {String} [options.log.type] A string value that indicates the console type to use when logging. Regular NodeJS 'console' methods.
   * @param {(Boolean | String)} [options.log.time] A string or boolean value that indicates a time format to use when logging (moment package format) or if to includes a default time format when logging.
   * @param {String} [options.log.tag] A string or boolean value that indicates a tag to use when logging or if to includes a tag at all
   * @param {...any} args Either the content or the settings and the content to log into the console. logger.figlet(settings, ...content) or logger.figlet(...content)
   * @returns {Logger} The Logger class
   ...
   *  };
   *
   *  logger.figlet(settings, 'Some text', 'More text', 'And even more text!!!');
   *  logger.figlet('Or just stock settings');
   */
  static figlet(...args) {
    ...
    const settings = {
      ...
      log: {
        ...
        options: {
          ...
          color: options.log && options.log.color ? options.log.color : options && options.color ? options.color : 'white',
   ...
   * @param {Object} [options] The Logger.log() method options
   * @see {@link https://www.npmjs.com/package/tools-kit|https://www.npmjs.com/package/tools-kit} For more log options
   * @param {String} [options.background] A string value that indicates the background color name to use when logging.
   * @param {String} [options.color] A string value that indicates the color name to use when logging.
   * @param {String} [options.style] A string value that indicates the style to use when logging.
   * @param {String} [options.type] A string value that indicates the console type to use when logging. Regular NodeJS 'console' methods.
   * @param {(Boolean | String)} [options.time] A string or boolean value that indicates a time format to use when logging (moment package format) or if to includes a default time format when logging.
   * @param {String} [options.tag] A string or boolean value that indicates a tag to use when logging or if to includes a tag at all
   * @param {...any} args Either the content or the settings and the content to log into the console. logger.log(settings, ...content) or logger.log(...content)
   * @returns {Logger} The Logger class
   ...
   *    type: 'log',
   *    time: true,
   *    tag: 'LOG WITH OPTIONS'
   *  };
   *
   *  logger.log(settings, 'Some text', 'More text', 'And even more text!!!');
   *  logger.log('Or just stock settings');
   */
  static log(...args) {
    const settings = util.isObject(args[0]) ? args[0] : false;
    const content = !settings ? args : args.slice(1);
